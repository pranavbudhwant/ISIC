...
from keras.layers import GlobalAveragePooling2D,GlobalMaxPooling2D,Input,\
                        ...
                        Conv2DTranspose,SpatialDropout2D
...
def build_decoder_densenet(latent_dim):
    ...
    #x = Dropout(0.5)(z_tensor)
    
    x = Dense(3136,activation='relu')(z_tensor) #x
    x = BatchNormalization()(x)
...
def build_decoder_densenet_v2():
    z_tensor = Input(shape=(7,7,1024))
    ...
    x = Conv2D(512,(3,3),strides=(1,1),padding='same')(z_tensor)
    x = BatchNormalization()(x)
    x = LeakyReLU(alpha=0.1)(x)
    x = SpatialDropout2D(0.2)(x)
